<div class="bg-white dark:bg-slate-800 shadow-lg rounded-sm border border-slate-200 dark:border-slate-700 mb-8">
    <header class="px-5 py-4">
        <h2 class="font-semibold text-slate-800 dark:text-slate-100">Invoices <span class="text-slate-400 dark:text-slate-500 font-medium">{{ $count }}</span></h2>
    </header>
                ...
                <tbody class="text-sm divide-y divide-slate-200 dark:divide-slate-700">
                    ...
                    @foreach($invoices as $invoice)
                        ...
                        <tr>
                            ...
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                <div class="font-medium {{$total_color}}">{{ $invoice->total }}</div>
                            </td>
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                <div class="inline-flex font-medium rounded-full text-center px-2.5 py-0.5 {{$status_color}}">{{ $invoice->status }}</div>
                            </td>
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                <div class="font-medium text-slate-800 dark:text-slate-100">{{ $invoice->customer }}</div>
                            </td>
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                <div>{{ \Carbon\Carbon::parse($invoice->issued_date)->format('d/m/Y') }}</div>
                            </td>
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                <div>@if($invoice->paid_date){{ \Carbon\Carbon::parse($invoice->paid_date)->format('d/m/Y') }}@else{{ '-' }}@endif</div>
                            </td>
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                <div class="flex items-center">
