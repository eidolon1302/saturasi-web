@php                    
    if ($campaign->type === 'One-Time') :
        $status_color = 'bg-emerald-100 dark:bg-emerald-400/30 text-emerald-600 dark:text-emerald-400';
    elseif ($campaign->type === 'At Risk') :
        $status_color = 'bg-amber-100 dark:bg-amber-400/30 text-amber-600 dark:text-amber-400';
    elseif ($campaign->type === 'Off-Track') :
        $status_color = 'bg-rose-100 dark:bg-rose-500/30 text-rose-500 dark:text-rose-400';
    else :
        $status_color = 'bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400';
    endif;
@endphp  
<div class="col-span-full sm:col-span-6 xl:col-span-4 bg-white dark:bg-slate-800 shadow-lg rounded-sm border border-slate-200 dark:border-slate-700">
    <div class="flex flex-col h-full p-5">
                ...
                <div class="flex shrink-0 -space-x-3 -ml-px">
                    @foreach ($campaign->members as $member)                        
                        <a class="block" href="{{ $member->link }}">
                            <img class="rounded-full border-2 border-white dark:border-slate-800 box-content" src="{{ asset('images/' . $member->image) }}" width="28" height="28" alt="{{ $member->name }}" />
                        </a>
                    @endforeach
                </div>
            </div>
        ...
        <div class="grow mt-2">
            <a class="inline-flex text-slate-800 dark:text-slate-100 hover:text-slate-900 dark:hover:text-white mb-1" href="#0">
                <h2 class="text-xl leading-snug font-semibold">{{ $campaign->title }}</h2>
            </a>
            <div class="text-sm">{{ $campaign->content }}</div>
        </div>
        <footer class="mt-5">
            <div class="text-sm font-medium text-slate-500 mb-2">{{ \Carbon\Carbon::parse($campaign->start_date)->format('M j') }} <span class="text-slate-400 dark:text-slate-600">-&gt;</span> {{ \Carbon\Carbon::parse($campaign->end_date)->format('M j') }}</div>
